...
class SMPP
{
    ...
    public function send_pdu($id, $data)
    {
        ...
        $pdu = pack('NNNN', mb_strlen($data) + 16, $id, 0, $this->seq).$data;
        // send PDU
        ...
        if ($this->debug) {
            echo "\n< R PDU (id,status,seq): ".implode(' ', $pdu);
        }
     ...
     */
    public function submit_sm($source_addr, $destination_addr, $short_message, $optional = ''): bool
    {
        ...
        $data .= sprintf('%c%s', mb_strlen($short_message), $short_message); // sm_length, short_message
        $data .= $optional;
     ...
     * @param  int  $flash
     */
    public function send_long($source_addr, $destination_addr, $short_message, $utf = 0, $flash = 0): bool
    {
        ...
        if ($size < 160) { // Only one part :)
            ...
            $sar_total_segments = ceil(mb_strlen($short_message) / 130);

            for ($sar_segment_seq_num = 1; $sar_segment_seq_num <= $sar_total_segments; $sar_segment_seq_num++) {
                $part          = mb_substr($short_message, 0, 130);
                $short_message = mb_substr($short_message, 130);
