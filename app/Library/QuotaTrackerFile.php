...
class QuotaTrackerFile
{
    ...
    public function check(?Carbon $timePoint = null)
    {
        ...
        $this->getSharedLock(function () use (&$result, $timePoint) {
            ...
            if (! is_null($this->quota)) {
                if ($this->quota['max'] !== self::UNLIMITED && $this->getUsage($timePoint) >= $this->quota['max']) {
                    $result = false;
    ...
    public function getUsage($timePoint = null, $interval = null): int
    {
        $this->getSharedLock(function () use (&$result, $timePoint, $interval) {
            if (is_null($interval)) {
                ...
                $result          = $value + count($this->getSeries($timePoint, $interval));
            } else {
                $result = count($this->getSeries($timePoint, $interval));
            }
        });
    ...
    {
        return (empty($this->series)) ? null : $this->series[count($this->series) - 1];
    }
