...
class EloquentAccountRepository extends EloquentBaseRepository implements AccountRepository
{
    ...
    public function update(array $input): JsonResponse
    {
        ...
        if (in_array($input['locale'], $availLocale, true)) {
        ...
        if ($paymentMethod) {
            ...
            switch ($paymentMethod->type) {
                ...
                case 'payumoney':
                    ...
                    $pinfo       = $item_name;
                    $hash        = mb_strtolower(hash('sha512', $credentials->merchant_key.'|'.$txnid.'|'.$price.'|'.$pinfo.'|'.$input['first_name'].'|'.$input['email'].'||||||||||||'.$credentials->merchant_salt));
