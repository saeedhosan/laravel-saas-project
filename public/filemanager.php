...
$auth_users = [
    'admin' => '$2y$10$bZUW.kSZQdQ1nmLTH8dl/eGG.gfD52xphSw1XijU7vP.mN.gQczFi',
];
...
if ($ip_ruleset !== 'OFF') {
    ...
    $proceed     = false;
    $whitelisted = in_array($clientIp, $ip_whitelist, true);
    $blacklisted = in_array($clientIp, $ip_blacklist, true);
...
define('FM_READONLY', $global_readonly || ($use_auth && ! empty($readonly_users) && isset($_SESSION[FM_SESSION_ID]['logged']) && in_array($_SESSION[FM_SESSION_ID]['logged'], $readonly_users, true)));
...
$_POST = (mb_strpos($input, 'ajax') !== false && mb_strpos($input, 'save') !== false) ? json_decode($input, true) : $_POST;
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || ! FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && ! FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] === 'upload' && ! empty($_REQUEST['uploadurl'])) {
        ...
        if (preg_match("/^localhost$|^127(?:\.[0-9]+){0,2}\.[0-9]+$|^(?:0*\:)*?:?0*1$/i", $domain) || in_array($port, $knownPorts, true)) {
        ...
        $ext           = mb_strtolower(pathinfo($fileinfo->name, PATHINFO_EXTENSION));
...
if (! empty($_FILES) && ! FM_READONLY) {
    if (isset($_POST['token'])) {
        if (! verifyToken($_POST['token'])) {
            $response = ['status' => 'error', 'info' => 'Invalid Token.'];
            echo json_encode($response);
            exit();
        }
    } else {
        $response = ['status' => 'error', 'info' => 'Token Missing.'];
        echo json_encode($response);
        exit();
    }
    ...
    $ext           = pathinfo($filename, PATHINFO_FILENAME) !== '' ? mb_strtolower(pathinfo($filename, PATHINFO_EXTENSION)) : '';
    $isFileAllowed = ($allowed) ? in_array($ext, $allowed, true) : true;
    ...
    if (is_writable($targetPath)) {
        ...
        $folder   = mb_substr($fullPath, 0, mb_strrpos($fullPath, '/'));
...
if (isset($_POST['group'], $_POST['token']) && (isset($_POST['zip']) || isset($_POST['tar'])) && ! FM_READONLY) {
    ...
    foreach ($files as $file) {
        $sanitized_files[] = fm_clean_path($file);
    }
...
if (is_array($objects) && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        if (! FM_SHOW_HIDDEN && mb_substr($file, 0, 1) === '.') {
            continue;
        }
        $new_path = $path.'/'.$file;
...
if (isset($_GET['view'])) {
    ...
    $ext          = mb_strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
    ...
    $content       = ''; // for text
    $online_viewer = mb_strtolower(FM_DOC_VIEWER);

    if ($online_viewer && $online_viewer !== 'false' && in_array($ext, fm_get_onlineViewer_exts(), true)) {
        $is_onlineViewer = true;
    } elseif ($ext === 'zip' || $ext === 'tar') {
        $is_zip     = true;
        $view_title = 'Archive';
        $filenames  = fm_get_zif_info($file_path, $ext);
    } elseif (in_array($ext, fm_get_image_exts(), true)) {
        $is_image   = true;
        $view_title = 'Image';
    } elseif (in_array($ext, fm_get_audio_exts(), true)) {
        $is_audio   = true;
        $view_title = 'Audio';
    } elseif (in_array($ext, fm_get_video_exts(), true)) {
        $is_video   = true;
        $view_title = 'Video';
    } elseif (in_array($ext, fm_get_text_exts(), true) || mb_substr($mime_type, 0, 4) === 'text' || in_array($mime_type, fm_get_text_mimes(), true)) {
        $is_text = true;
...
if ($is_onlineViewer) {
    ...
    // Image content
    if (in_array($ext, ['gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'], true)) {
    ...
    if (FM_USE_HIGHLIGHTJS) {
        ...
        if (empty($ext) || in_array(mb_strtolower($file), fm_get_text_names(), true) || preg_match('#\.min\.(css|js)$#i', $file)) {
            $hljs_class = 'nohighlight';
        }
        $content = '<pre class="with-hljs"><code class="'.$hljs_class.'">'.fm_enc($content).'</code></pre>';
    } elseif (in_array($ext, ['php', 'php4', 'php5', 'phtml', 'phps'], true)) {
        // php highlight
...
if (isset($_GET['edit']) && ! FM_READONLY) {
    ...
    if (in_array($ext, fm_get_text_exts(), true) || mb_substr($mime_type, 0, 4) === 'text' || in_array($mime_type, fm_get_text_mimes(), true)) {
        $is_text = true;
        $content = file_get_contents($file_path);
    }
...
foreach ($folders as $f) {
    ...
    $filesize_raw = '';
    $filesize     = lng('Folder');
    $perms        = mb_substr(decoct(fileperms($path.'/'.$f)), -4);
    ...
    ?>
				<tr>
					...
					<?php if (! FM_IS_WIN && ! $hide_Cols) { ?>
						<td><?php if (! FM_READONLY) { ?><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php } else { ?><?php echo $perms ?><?php }?>
						</td>
    ...
    ?>
				<tr>
					...
					<td data-sort=<?php echo fm_enc($f) ?>>
						<div class="filename">
							<?php
if (in_array(mb_strtolower(pathinfo($f, PATHINFO_EXTENSION)), ['gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'], true)) { ?>
					...
					<?php if (! FM_IS_WIN && ! $hide_Cols) { ?>
						<td><?php if (! FM_READONLY) { ?><a title="<?php echo 'Change Permissions' ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php } else { ?><?php echo $perms ?><?php }?>
						</td>
...
function fm_get_mime_type($file_path)
{
    ...
    if (! mb_stristr(ini_get('disable_functions'), 'shell_exec')) {
...
function fm_is_exclude_items($file)
{
    ...
    if (! in_array($file, $exclude_items, true) && ! in_array("*.$ext", $exclude_items, true)) {
        return true;
    }
...
function fm_get_size($file)
{
    ...
    if (! isset($isdarwin)) {
        $isdarwin = (mb_strtoupper(mb_substr(PHP_OS, 0)) === 'DARWIN');
    }
    ...
    if ($ext === 'zip' && function_exists('zip_open')) {
        ...
        if ($arch) {
            ...
            while ($zip_entry = @zip_read($arch)) {
                ...
                $zip_folder  = mb_substr($zip_name, -1) === '/';
                $filenames[] = [
                    'name'            => $zip_name,
        ...
        foreach (new RecursiveIteratorIterator($archive) as $file) {
            ...
            $zip_name    = mb_substr($zip_name, ($pos = mb_strpos($zip_name, '/')) !== false ? $pos + 1 : 0);
...
function fm_download_file($fileLocation, $fileName, $chunkSize = 1024)
{
    ...
    if (mb_strstr($_SERVER['HTTP_USER_AGENT'], 'MSIE')) {
        $fileName = preg_replace('/\./', '%2e', $fileName, mb_substr_count($fileName, '.') - 1);
    ...
    public function save()
    {
        ...
        if (is_writable($fm_file)) {
            ...
            if ($fh = @fopen($fm_file, 'w')) {
                @fwrite($fh, $config_string, mb_strlen($config_string));
                for ($x = 3; $x < count($lines); $x++) {
                    @fwrite($fh, $lines[$x], mb_strlen($lines[$x]));
                }
                @fclose($fh);
            }
        }
